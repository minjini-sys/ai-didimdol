export function applySafetyGate(route, matches) {
  ...
  if (route.riskLevel === "blocked") {
    status = "block";
    warnings.push("불법, 계정 탈취, 비밀번호 요청처럼 도와줄 수 없는 내용이 포함되어 있습니다.");
  }

  if (route.riskLevel === "high") {
    status = status === "block" ? status : "confirm";
    warnings.push("개인정보, 돈, 의료, 사기 가능성이 있어 확인이 필요합니다.");
  ...
  if (status !== "allow" && matches.mcps.some((mcp) => mcp.privacyRisk === "high")) {
